/**
 * Social.js
 * @description Used to define the user's social status on Microsoft SharePoint, requires jQuery;
 * @author Wilfredo Paheco
 */

/**
 * Social
 * @param {string} SP_SITE Foot url;
 * @param {object} Route Method used to call REST Api;
 * @returns Object of methods used to get the user's social details;
 */
export default function Social({SP_SITE, service}){
    return {

        /** Sites & Documents; */
        getFollowing: function getFollowing(){
            return service.get(`${SP_SITE}/_api/social.following`)
            .then(data => data.d);
        },
        
        /** Get the people who are following the current user; */
        getFollowers: function getFollowers(){
            return service.get(`${SP_SITE}/_api/social.following/my/Followers`)
            .then(data => data.d);
        },
        ...
        getSuggestions: function getSuggestions(){
            return service.get(`${SP_SITE}/_api/social.following/my/Suggestions`)
            .then(data => data.d);
        },
