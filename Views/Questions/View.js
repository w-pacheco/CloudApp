...
import { Title as SeriesTitle } from '../Series/List.js';
...
function correctSeriesTitles(event){
    
    const index = getTableHeaderIndex('Series_FK');
    const List = Web.getListDetails(SeriesTitle);
    const Series = store.listObserver.getListDataById(List.Id);
    ...
    .map(el => {
        if (el.anCells[index].innerText)
        {
            ...
            {
                const project_fk = el.anCells[index].innerText;
                const result = Series.find(p => p.GUID === project_fk);
                el.anCells[index].innerText = result.Project;
            }
            catch(e){ }
        }
...
export const Icon = 'question-circle-fill';
export const th = [{
    thead: 'Id',
    col: {
        data: 'Id',
...
/** Admin View Method; */
export function QuestionsTab(menu, tabcontent){
...
export default function Questions() {
    ...
    /** FIXME: This needs to be updated and handled correctly; */
    // table.$table.on('draw', correctSeriesTitles);
