...
import ColorPicker from "../src/Classes/ColorPicker.js";
...
export function DarkThemeBtn(name = 'brightness-high'){
    ...
    return new Component({
        ...
        attributes: [{ name: 'title', value: 'Dark_Mode' }],
        events: [{
            name: 'click', 
            action(event){
...
export function CustomConsole(name = 'terminal'){
    ...
    return new Component({
        tag: 'button',
        classList: 'btn text-darkColor',
        attributes: [{ name: 'title', value: 'Virtual_Console' }],
        events: [{
            name: 'click', 
            action(event){
                if (window?.ReportErrors) return window.ReportErrors();
                else return alert('This mehod has not been loaded!');
            },
        }],
        innerHTML: /*html*/`${icon.outerHTML}&#160;Console`,
    });
...
export function ClearStateBtn(name = 'lightning'){
    ...
    return new Component({
        ...
        innerHTML: /*html*/`${icon.outerHTML}&#160;Reset Tables`,
    });
...
export default function Settings(){
    ...
    CustomConsole().render(buttonGroup);
