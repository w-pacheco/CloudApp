...
import App, { Layout, User, Web } from "../../app.js";
...
/** Admin View Method; */
export function ProjectsTab(menu, tabcontent){
...
export default function Projects(){
    ...
    /** Filter only the projects the user is a member of; */
    let $filter;
    if (!User.hasRole(DEVELOPER) && !User.hasRole(ADMINISTRATOR)) $filter = User.Projects.map(function({ GUID }){
        return `(GUID eq '${GUID}')`;
    }).join(' or ');
    // $filter: User.Projects.map(function({ Id }){
    //     return `(Id eq ${Id})`;
    // }).join(' or '),
